...
from eval.robot_controller_rtc import RobotController
...
def main(args: Args) -> None:
    ...
    # Fake input for compilation
    compile_start_time = time.time()
    obs = {
        'images': {
            'left_wrist_view': np.zeros((480,640,3)),
            'face_view': np.zeros((480,640,3)),
            'right_wrist_view': np.zeros((480,640,3)),
        },
        'prompt': 'sort and fold clothes',
        'state': np.zeros(16),
    }
    norm_prev_chunk = np.zeros((args.action_horizon, 14))
    result = policy.infer_rtc(obs, norm_prev_chunk, args.inference_delay, args.prefix_attention_horizon, args.max_guidance_weight[0])
    print(f'compile time: {time.time() - compile_start_time}')
    ...
    def _pred_func(self, views, actions) -> dict:
        ...
        if not hasattr(self, 'prev_chunk'):
            self.prev_chunk = np.zeros((args.action_horizon, 14))
        if not hasattr(self, 'prev_action_pred_abs'):
            self.prev_action_pred_abs = np.zeros((args.action_horizon, 14))
        obs_list = []
        for key in obs_action_keys:
        ...
        # Update prev chunk (We need to know the current_agent_pos to update the prev_chunk)
        current_agent_pos = agent_pos
        next_inference_start_EE_traj = self.prev_action_pred_abs[args.execution_horizon-1 :]
        next_inference_start_EE_traj[0] = current_agent_pos[0:14]
        ...
        self.prev_chunk = np.concatenate([
        ...
        norm_prev_chunk = normalize_action(self.prev_chunk, predict_action_min, predict_action_max)
        result = policy.infer_rtc(obs, norm_prev_chunk, args.inference_delay, args.prefix_attention_horizon, args.max_guidance_weight[0])
        end_time = time.time()
        ...
        self.prev_action_pred_abs = action_pred
